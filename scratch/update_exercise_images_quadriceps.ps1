...
Write-Output "Iniciando atualização de imagens de Quadríceps no Supabase..."
...
foreach ($item in $res) {
    if ($item.data -and $item.data.name) {
        ...
        if ($nameLower -like "*agachamento*livre*" -or $nameLower -eq "agachamento livre com barra") {
            $imageUrl = "assets/exercises/barbell_squat.png"
        }
        elseif ($nameLower -like "*cadeira extensora*" -or $nameLower -like "*extensora*") {
            $imageUrl = "assets/exercises/leg_extension.png"
        }
        elseif ($nameLower -like "*leg press*") {
            $imageUrl = "assets/exercises/leg_press_45.png"
        }
        elseif ($nameLower -like "*agachamento*b*lgaro*" -or $nameLower -like "*bulgaro*") {
...
Write-Output "Atualização de imagens de Quadríceps concluída!"
