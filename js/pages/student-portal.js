...
export async function renderStudentPortal(rawParam) {
  ...
  if (student?.name) localStorage.setItem(`portal_name_`, student.name);

  const sessionKey = `portal_auth_`;
    ...
    <div class="portal-section">
      ...
      ${currentMacro ? `
        <div class="glass-card portal-macro-card">
          ...
          <div class="portal-macro-pct mb-md">${macroProgress}% concluÃ­do &middot; ${macroSessionsCount} sessÃµes no ciclo</div>
            ...
            <div style="margin-top:16px; border-top:1px solid rgba(255,255,255,0.08); padding-top:12px">
              ...
              <div style="display:grid; grid-template-columns:repeat(auto-fill, minmax(70px, 1fr)); gap:6px">
                ${currentMacro.weeks.map(w => {
                  ...
                  const weekStartPct = (weekIndex / currentMacro.weeks.length) * 100;
                  const weekEndPct = ((weekIndex + 1) / currentMacro.weeks.length) * 100;
                  const isCurrent = (macroProgress >= weekStartPct && macroProgress < weekEndPct) || (macroProgress >= 100 && weekIndex === currentMacro.weeks.length - 1);
                  const isPast = macroProgress >= weekEndPct && macroProgress < 100;
                  return `
                  <div style="
