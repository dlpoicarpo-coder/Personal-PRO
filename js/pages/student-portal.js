// ========================================
// VETOR — Student Portal (PWA Mobile)
  ...
  const suggestedCard = suggestedWorkout ? `
    <div class="portal-suggested-card" id="suggestedCard" style="--wk-rgb: ${getWorkoutTheme(suggestedWorkout.name, suggestedWorkout.category).rgb}; background: linear-gradient(135deg, rgba(var(--wk-rgb),0.15), rgba(var(--wk-rgb),0.03)); border-color: rgba(var(--wk-rgb), 0.3)">
      ...
      <div style="display:flex;gap:10px;margin-top:12px;flex-wrap:wrap">
        ...
        <button class="portal-submit-btn" id="startSuggestedBtn" data-wid="${suggestedWorkout.id}" style="margin:0;flex:1;min-width:130px;background:linear-gradient(135deg, rgba(var(--wk-rgb), 1), rgba(var(--wk-rgb), 0.8));font-size:0.82rem;padding:10px 14px">
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="5 3 19 12 5 21 5 3"/></svg>
          Iniciar Treino
        </button>
      </div>
  ...
  return `
    <div class="portal-section">
      ...
      <div id="soloSetupContainer" style="${running ? 'display:none' : ''}">
        ...
        <button id="soloStartBtn" class="portal-submit-btn" style="display:none;background:linear-gradient(135deg,#6366f1,#8b5cf6);box-shadow:0 4px 16px rgba(99,102,241,0.3)">
...
function initTreinar(workouts, schedules, student, sessions = []) {
  ...
  document.querySelectorAll('.portal-workout-pick-item').forEach(card => {
    card.addEventListener('click', () => {
      ...
      const targetW = w || { id: '', name: 'Livre', category: 'Livre', exercises: [] };
      showWorkoutSheet(targetW);
    });
  });
...
function initCardioChart(segments, isTimeSpeed) {
  const ctx = document.getElementById('cardioIntervalChart')?.getContext('2d');
  if (!ctx) return;
...
async function showExerciseModal(ex) {
  ...
  // Cardio checks and segments
  const isCardio = isCardioExercise(ex);
  const isTimeSpeed = isSpeedPowerCardio(ex);
  const segments = isCardio ? getCardioSegments(ex) : [];
  const totalSec = segments.reduce((sum, seg) => sum + seg.duration, 0);
  ...
  if (isCardio && typeof Chart !== 'undefined') {
    setTimeout(() => {
      initCardioChart(segments, isTimeSpeed);
    }, 100);
  }
...
window.showWorkoutSheet = function(w) {
  const container = document.querySelector('.portal-root') || document.body;
  ...
  const cat = w.category || 'Livre';
  const theme = getWorkoutTheme(w.name, cat); // We reuse the theme logic

  const sheet = document.createElement('div');
  sheet.style.cssText = `
    background: #0f172a; width: 100%; max-height: 90vh;
  ...
  if (w.exercises && w.exercises.length > 0) {
    exercisesHtml = w.exercises.map((ex,i) => {
      const isCardio = isCardioExercise(ex);
      let cardioHtml = '';
      if (isCardio && typeof getCardioSegments === 'function') {
        const segments = getCardioSegments(ex);
  ...
  sheet.innerHTML = `
    ...
    <div style="display:flex; align-items:center; justify-content:space-between; z-index:1;">
      ...
      <button class="sheet-close-btn" style="background:none; border:none; color:#94a3b8; cursor:pointer; padding:8px; border-radius:50%;">
  ...
  sheet.querySelectorAll('.portal-ex-pick-card').forEach(card => {
    card.addEventListener('click', async () => {
      ...
      const ex = w.exercises[ei];
      if (ex && typeof window.showExerciseModal === 'function') {
        await window.showExerciseModal(ex);
      }
    });
  });
};
