...
window.generateConsentLink = (studentId, phone, guardianEmail, studentName, trainerId, btn) => {
  const link = `${window.location.origin}${window.location.pathname}#/form/consentimento?student=${studentId}&t=${trainerId}`;
  const firstName = (studentName || 'Aluno').split(' ')[0];
  const msg = `Ola! Para continuarmos o acompanhamento de ${firstName}, por favor acesse o link abaixo e preencha o Termo de Consentimento LGPD: ${link}`;
  ...
  let emailBtn = '';
  if (guardianEmail) {
    const mailtoHref = `mailto:${guardianEmail}?subject=${encodeURIComponent('Termo de Consentimento LGPD - ' + studentName)}&body=${encodeURIComponent(msg)}`;
    emailBtn = `<a href="${mailtoHref}" target="_blank" class="btn btn-secondary btn-sm" style="padding:4px 8px;display:inline-flex;align-items:center;gap:4px"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg> E-mail</a>`;
  }

  const copyBtn = `<button class="btn btn-secondary btn-sm" style="padding:4px 8px;display:inline-flex;align-items:center;gap:4px" onclick="navigator.clipboard.writeText('${link}'); window.notify?.success ? window.notify.success('Link copiado!') : alert('Link copiado!')"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg> Copiar Link</button>`;

  const container = document.getElementById(`consentBox_${studentId}`);
  if (container) {
    container.innerHTML = `<div class="flex gap-xs items-center" style="flex-wrap:wrap">${copyBtn}${waBtn}${emailBtn}</div>`;
  }
};
  ...
  return `
    <div class="flex items-center gap-lg mb-lg" style="flex-wrap:wrap">
      ...
      <div class="flex gap-sm" style="flex-wrap:wrap;align-items:center">
        ...
        <a href="#/portal/${student.id}?t=${trainerId}" target="_blank" class="btn btn-ghost btn-sm" style="padding:4px 8px">Ver portal</a>
        ...
        label: 'Salvar', class: 'btn-primary', id: 'saveStudent', onClick: async () => {
          ...
          if (!data.code) {
            ...
            do {
              ...
              num++;
            } while (allStudents.some(s => s.code === code));
            data.code = code;
          }
