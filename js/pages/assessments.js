  ...
  return `
    ...
    <div id="panel-composicao" class="assessment-panel">
      ${!compAss.length ? `
        ...
        <!-- Card de calorias — aparece ao selecionar aluno no filtro -->
        <div id="assKcalCard" class="card mt-lg" style="display:none;border-left:3px solid var(--primary)"></div>
      `}
    </div>
...
export function initAssessments(navigateFn) {
  ...
  // Filtro por aluno — filtra tabela E mostra calorias
  document.getElementById('assStudentFilter')?.addEventListener('change', async e=>{
    ...
    // Mostrar/atualizar card de calorias
    const kcalCard = document.getElementById('assKcalCard');
    if (!sid) { if(kcalCard) kcalCard.style.display='none'; return; }
    const [students, assessments] = await Promise.all([db.getAll('students'), db.getAll('assessments')]);
    const st     = students.find(s=>s.id===sid);
    const compAss= assessments.filter(a=>a.studentId===sid&&a.type==='composicao').sort((a,b)=>new Date(b.date)-new Date(a.date));
    const lastC  = compAss[0];
    if (!lastC?.peso || !st) { if(kcalCard) kcalCard.style.display='none'; return; }
    ...
    if (!tmb2) { if(kcalCard) kcalCard.style.display='none'; return; }
    ...
    const oc     = obj.includes('emagr')?'var(--warning)':obj.includes('hipert')?'var(--success)':'var(--accent)';
    const el     = document.getElementById('assKcalCard');
    if (el) {
      el.style.display='';
      el.innerHTML = `
        <div class="card-header">
          <span class="card-title">Gasto Energético — ${st.name}</span>
          <span class="text-xs text-muted">${tmb2.formula} · avaliação de ${Calc.formatDate(lastC.date)}</span>
        </div>
        <div class="stats-grid" style="grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:12px">
          ${[['TMB',tmb2.valor+'kcal','var(--text-secondary)',tmb2.formula],['TDEE',tdee2.valor+'kcal','var(--primary)','×'+tdee2.fator+' (moderado)'],['Meta',meta2.kcal+'kcal',oc,meta2.label]].map(([l,v,c,s])=>`
            <div class="stat-card" style="text-align:center;padding:10px">
              <div class="stat-label">${l}</div>
              <div style="font-size:1.2rem;font-weight:800;color:${c}">${v}</div>
              <div style="font-size:0.63rem;color:var(--text-muted);margin-top:2px">${s}</div>
            </div>`).join('')}
        </div>
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px">
          ${[['Proteína',mac2.proteina,'#10b981'],['Carboidrato',mac2.carboidrato,'#f59e0b'],['Gordura',mac2.gordura,'#8b5cf6']].map(([n,m,c])=>`
            <div style="padding:10px 12px;background:var(--bg-page);border-radius:8px;border-left:3px solid ${c}">
              <div style="font-size:0.68rem;color:var(--text-muted);text-transform:uppercase">${n}</div>
              <div style="font-size:1.2rem;font-weight:700;color:${c}">${m.g}g</div>
              <div style="font-size:0.68rem;color:var(--text-muted)">${m.kcal}kcal · ${m.pct}%</div>
            </div>`).join('')}
        </div>
        <div style="margin-top:8px;font-size:0.7rem;color:var(--text-muted)">
          Proteína: <strong>${mac2.protPorKg}g/kg</strong> (ISSN 2018) · Objetivo: ${st.goal||'Geral'}
        </div>`;
    }
  });
