// ========================================
// VETOR — Biofeedback Page (v5)
// Design limpo · Radar PSE · Gráficos · Múltiplas regiões de dor · Excluir
// ========================================
        ...
        <div style="display:flex;align-items:flex-start;gap:10px;padding:8px 0;border-bottom:1px solid var(--border-color)">
          ...
          <div>
            ...
            <span style="color:var(--danger)"> — ${
              ...
              a.metric === 'Ciclo Menstrual' ? `${a.metric}: ${a.value}` :
              a.metric === 'Dor Localizada' ? `${a.metric}` :
              `${a.metric}: ${a.value}/10`
            }</span>
...
function renderBfContent(entries, students, filterStudentId, limitOverride = 30, periodFilter = 'month') {
  ...
  // Radar PSE — Prontidão e Carga por aluno (substitui ACWR)
  let radarSection = '';
  if (student) {
    const allSessions = await db.getAll('sessions');
    const completedSessions = allSessions.filter(s => s.status === 'completed');
    const r = computeReadiness(student.id, entries, student.name);
    const l = computeLoad(student.id, completedSessions, student.name);

    const renderRadarCard = (res) => {
      const isMaturing = res.status === 'collecting' || res.status === 'maturing';
      const levelColor = res.level === 'danger' ? 'var(--danger)' : res.level === 'warning' ? 'var(--warning)' : 'var(--success)';
      const kindLabel = res.kind === 'load' ? 'Carga de Treino' : 'Prontidão';
      const kindIcon = res.kind === 'load'
        ? `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:middle;margin-right:5px"><path d="M22 12h-4l-3 9L9 3l-3 9H2"/></svg>`
      ...
      const metricsHtml = (res.metrics || []).map(m => {
        const labels = { sleep: 'Sono', tqr: 'Recuperação (TQR)', stress: 'Estresse', pain: 'Dor', trainingLoad: 'Carga' };
        const mColor = m.level === 'danger' ? 'var(--danger)' : m.level === 'warning' ? 'var(--warning)' : 'var(--text-muted)';
        const todayVal = m.name === 'trainingLoad' ? Math.round(m.today) : m.today;
        const meanVal = m.name === 'trainingLoad' ? Math.round(m.mean) : m.mean?.toFixed(1);
        return `<div style="display:flex;justify-content:space-between;align-items:center;padding:4px 0;border-bottom:1px solid var(--border-color);font-size:0.8rem">
          <span style="color:var(--text-muted)">${labels[m.name] || m.name}</span>
          <span style="font-weight:600;color:${mColor}">${todayVal} <span style="color:var(--text-muted);font-weight:400">/ média ${meanVal}</span></span>
        </div>`;
      }).join('');

      const progressHtml = isMaturing && res.progress
        ? `<div style="margin-top:8px"><div style="height:4px;background:var(--border-color);border-radius:2px"><div style="height:4px;width:${Math.min(100, Math.round((res.progress.have / res.progress.need) * 100))}%;background:var(--primary);border-radius:2px"></div></div><div class="text-xs text-muted mt-xs">${res.progress.have}/${res.progress.need} ${res.kind === 'load' ? 'semanas' : 'check-ins'}</div></div>`
        : '';

      return `<div style="flex:1;min-width:0">
        <div style="display:flex;align-items:center;gap:6px;margin-bottom:6px">
          <span style="font-size:0.75rem;font-weight:700;text-transform:uppercase;letter-spacing:.04em;color:var(--text-muted)">${kindIcon}${kindLabel}</span>
          ${!isMaturing ? `<span style="font-size:0.72rem;font-weight:600;padding:2px 7px;border-radius:20px;background:${levelColor}18;color:${levelColor}">${res.headline}</span>` : `<span class="text-xs text-muted">${res.detail}</span>`}
        </div>
        ${metricsHtml}
        ${progressHtml}
        ${!isMaturing && res.detail ? `<p class="text-xs text-muted mt-xs" style="font-style:italic">${res.detail}</p>` : ''}
      </div>`;
    };

    radarSection = `
      <div class="card mb-lg">
        <div class="card-header"><span class="card-title">Radar PSE — Prontidão e Carga (${student.name})</span></div>
        ...
        <table class="data-table">
          ...
          <tbody>${recent.map(e => {
            ...
            return `<tr>
              ...
              <td style="font-weight:600;color:${colorForVal(e.sleep,false)}">${e.sleep ? `${Math.round(e.sleep / 2)}/5` : '-'}</td>
              ...
              <td style="color:${colorForVal(e.pain,true)}">
                ${e.pain ? `${e.pain > 8 ? 5 : e.pain > 6 ? 4 : e.pain > 4 ? 3 : e.pain > 2 ? 2 : 1}/5` : '-'}
              ...
              <td style="font-weight:600;color:${colorForVal(e.motivation,false)}">${e.motivation ? `${Math.round(e.motivation / 2)}/5` : '-'}</td>
    ...
    openModal({
      ...
      actions: [
        ...
        { label:'Salvar', class:'btn-primary', onClick: async () => {
          ...
          ['sleep','tqr','stress','pain','food','pse','duration','motivation'].forEach(k=>d[k]=parseInt(d[k])||((k==='motivation'||k==='sleep')?4:0));
          
          // Map 1-5 UI scale to database 10-point scale
          d.sleep = d.sleep * 2;
          d.motivation = d.motivation * 2;
...
function bindBfActions(navigateFn, studentsCache) {
    ...
    btn.addEventListener('click', async () => {
      ...
      const html = `
        ...
        <div style="margin-bottom:16px">
          ...
          ${entry.pain >= 3 ? `
            <div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid var(--border-color);font-size:0.85rem">
              <span class="text-muted" style="display:flex;align-items:center">${ICON_PIN} Local da Dor</span>
  ...
  document.querySelectorAll('.edit-bf').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      const sleepUiVal = Math.round((entry.sleep || 8) / 2);
      const motivationUiVal = Math.round((entry.motivation || 8) / 2);
      ...
      openModal({
        ...
        actions: [
          ...
          { label: 'Salvar Alterações', class: 'btn-primary', onClick: async () => {
            ...
            const sleepUiVal = parseInt(fd.get('sleep')) || 4;
            const motivationUiVal = parseInt(fd.get('motivation')) || 4;
            const painUiVal = parseInt(fd.get('pain')) || 1;

            const sleepVal = sleepUiVal * 2;
            const motivationVal = motivationUiVal * 2;
            const painDbMap = [0, 1, 3, 5, 7, 10];
            const painVal = painDbMap[painUiVal] || 1;
            ...
            const updated = {
              ...
              pain: painVal,
              painRegions: painVal >= 3 ? selectedPainRegs : [],
