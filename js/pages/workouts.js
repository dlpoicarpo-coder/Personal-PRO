...
export function getWorkoutBaseName(name) {
  if (!name) return '';
  return name
    .replace(/\s*[-â€”_:\(]?\s*\b(?:sem|semana|sem\.|s|w|week)\s*\d+\b\)?/gi, '')
    .trim();
}
...
export async function renderWorkouts() {
  ...
  // Extrair semanas e nomes base Ãºnicos
  ...
  const cycleOptions = macros.map(m => {
    ...
    return `<option value="${m.id}" data-student="${m.studentId}" ${selected}>${st ? st.name.split(' ')[0] : '?'} â€” ${m.name}</option>`;
  }).join('');

  return `
    <div class="page-header">
      <div>
        <h1>PrescriÃ§Ã£o de Treinos</h1>
    ...
    <div id="workoutsList">
      ${workouts.length ? `
        <div class="table-container">
          <table class="data-table">
            <thead><tr>
              <th>Aluno</th><th>Treino</th><th>Data</th><th>Fase</th><th>ExercÃ­cios</th><th>Semana</th><th></th>
            </tr></thead>
            <tbody>
              ${workouts.map(w => {
                ...
                return `<tr data-student="${w.studentId}" data-macroid="${w.macrocycleId || ''}" data-name="${(w.name||'').toLowerCase()}" data-week="${weekStr}" data-basename="${baseNameStr}">
                  ...
                  <td>
                    <div style="font-weight:600;font-size:0.88rem;display:flex;align-items:center;gap:4px">
                      ${w.name || 'Treino'}
                      ${isRealizado ? `<span class="badge badge-success" style="font-size:0.6rem;padding:2px 6px;text-transform:none">âœ“ Realizado (${doneSessions.length})</span>` : ''}
                    </div>
      ...
      ` : `
        <div class="empty-state">
          ...
          <p>Crie o primeiro treino ou gere via PeriodizaÃ§Ã£o</p>
    ...
    <form id="workoutForm">
      ...
      <div class="form-row">
        ...
        <div class="form-group">
          ...
          <input class="form-input" name="cycle" value="${workout.cycle||''}" placeholder="Ex: Ciclo 1 - AdaptaÃ§Ã£o" />
        </div>
      </div>
      <div class="form-group">
        <label class="form-label">ObservaÃ§Ãµes</label>
        <textarea class="form-textarea" name="notes" rows="2" placeholder="OrientaÃ§Ãµes gerais...">${workout.notes||''}</textarea>
      </div>
      <div style="border-top:1px solid var(--border-color); padding-top:16px; margin-top:16px">
        <div class="flex items-center justify-between mb-md">
          <h4>ExercÃ­cios</h4>
          <button type="button" class="btn btn-secondary btn-sm" id="addExerciseRow">+ ExercÃ­cio</button>
        </div>
        <div id="exerciseRows">
...
// â”€â”€ DEFINIÃ‡ÃƒO DE PROGRESSÃƒO POR MÃ‰TODO â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// MÃ©todos que geram mÃºltiplas sub-sÃ©ries com reps/carga diferente por sÃ©rie
export const METHOD_PROGRESSIONS = {
  'PirÃ¢mide Crescente': {
    desc: 'DeLorme: carga sobe a cada sÃ©rie, reps caem. Aquecimento progressivo â†’ pico de intensidade. 90-120s descanso.',
    series: [
      { reps: '12-15', loadPct: 0.60, label: 'S1 â€” Leve',        rest: 90  },
      { reps: '10-12', loadPct: 0.70, label: 'S2 â€” Moderada',    rest: 90  },
      { reps: '8-10',  loadPct: 0.80, label: 'S3 â€” Pesada',      rest: 120 },
      { reps: '6-8',   loadPct: 0.88, label: 'S4 â€” Muito Pesada',rest: 0   },
    ]
  },
  'PirÃ¢mide Decrescente': {
    desc: 'Oxford: inicia pesado sem prÃ©-fadiga â†’ carga cai, volume sobe. 120-180s descanso nos sets pesados.',
    series: [
      { reps: '4-6',   loadPct: 0.88, label: 'S1 â€” MÃ¡ximo',   rest: 180 },
      { reps: '6-8',   loadPct: 0.80, label: 'S2 â€” Pesada',   rest: 150 },
      { reps: '8-10',  loadPct: 0.72, label: 'S3 â€” Moderada', rest: 120 },
      { reps: 'AMRAP', loadPct: 0.67, label: 'S4 â€” Leve/MÃ¡x', rest: 0   },
    ]
  },
  'PirÃ¢mide Dupla': {
    desc: 'Crescente atÃ© o pico, depois decrescente. SÃ©ries de descida em AMRAP para avanÃ§ados. 90-120s descanso.',
    series: [
      { reps: '15',    loadPct: 0.60, label: 'S1 â€” Base',     rest: 90  },
      { reps: '12',    loadPct: 0.68, label: 'S2 â€” Leve',     rest: 90  },
      { reps: '10',    loadPct: 0.75, label: 'S3 â€” Moderada', rest: 120 },
      { reps: '6-8',   loadPct: 0.85, label: 'S4 â€” Pico â†‘',  rest: 120 },
      { reps: 'AMRAP', loadPct: 0.75, label: 'S5 â€” Moderada', rest: 90  },
      { reps: 'AMRAP', loadPct: 0.68, label: 'S6 â€” Leve',     rest: 90  },
      { reps: 'AMRAP', loadPct: 0.60, label: 'S7 â€” Base â†“',   rest: 0   },
    ]
  },
  'PirÃ¢mide Completa': {
    desc: 'PirÃ¢mide dupla com pico duplo. 8 sÃ©ries. Descida em AMRAP. 90-150s descanso. IntermediÃ¡rios/AvanÃ§ados.',
    series: [
      { reps: '15',    loadPct: 0.60, label: 'S1 â€” Base',      rest: 90  },
      { reps: '12',    loadPct: 0.68, label: 'S2 â€” Leve',      rest: 90  },
      { reps: '10',    loadPct: 0.75, label: 'S3 â€” Moderada',  rest: 120 },
      { reps: '8',     loadPct: 0.82, label: 'S4 â€” Pesada',    rest: 120 },
      { reps: '6',     loadPct: 0.88, label: 'S5 â€” Pico â†‘',   rest: 150 },
      { reps: '8',     loadPct: 0.82, label: 'S6 â€” Pesada',    rest: 120 },
      { reps: 'AMRAP', loadPct: 0.75, label: 'S7 â€” Moderada',  rest: 90  },
      { reps: 'AMRAP', loadPct: 0.65, label: 'S8 â€” Base â†“',   rest: 0   },
    ]
  },
  'Drop-set': {
    desc: '75% 1RM atÃ© falha â†’ -20% sem pausa â†’ -20% sem pausa. 2-3min descanso apÃ³s cada drop-set completo.',
    series: [
      // SÃ©rie 1
      { reps: 'atÃ© falha', loadPct: 0.75, label: 'S1 â€” Principal',   rest: 5   },
      { reps: 'atÃ© falha', loadPct: 0.60, label: 'S1 â€” Drop 1 -20%', rest: 5   },
      { reps: 'atÃ© falha', loadPct: 0.48, label: 'S1 â€” Drop 2 -20%', rest: 120 },
      // SÃ©rie 2
      { reps: 'atÃ© falha', loadPct: 0.75, label: 'S2 â€” Principal',   rest: 5   },
      { reps: 'atÃ© falha', loadPct: 0.60, label: 'S2 â€” Drop 1 -20%', rest: 5   },
      { reps: 'atÃ© falha', loadPct: 0.48, label: 'S2 â€” Drop 2 -20%', rest: 120 },
      // SÃ©rie 3
      { reps: 'atÃ© falha', loadPct: 0.75, label: 'S3 â€” Principal',   rest: 5   },
      { reps: 'atÃ© falha', loadPct: 0.60, label: 'S3 â€” Drop 1 -20%', rest: 5   },
      { reps: 'atÃ© falha', loadPct: 0.48, label: 'S3 â€” Drop 2 -20%', rest: 0   },
    ]
  },
  'Stripping': {
    desc: 'Drop-set com barra â€” remover anilhas sem parar. 3-4 strips de -20% por sÃ©rie. 3 sÃ©ries com 120s descanso.',
    series: [
      // SÃ©rie 1
      { reps: 'atÃ© falha', loadPct: 0.80, label: 'S1 â€” Carga mÃ¡x.',   rest: 5   },
      { reps: 'atÃ© falha', loadPct: 0.62, label: 'S1 â€” Strip 1 -22%', rest: 5   },
      { reps: 'atÃ© falha', loadPct: 0.48, label: 'S1 â€” Strip 2 -22%', rest: 5   },
      { reps: 'atÃ© falha', loadPct: 0.37, label: 'S1 â€” Strip 3 -22%', rest: 120 },
      // SÃ©rie 2
      { reps: 'atÃ© falha', loadPct: 0.80, label: 'S2 â€” Carga mÃ¡x.',   rest: 5   },
      { reps: 'atÃ© falha', loadPct: 0.62, label: 'S2 â€” Strip 1 -22%', rest: 5   },
      { reps: 'atÃ© falha', loadPct: 0.48, label: 'S2 â€” Strip 2 -22%', rest: 5   },
      { reps: 'atÃ© falha', loadPct: 0.37, label: 'S2 â€” Strip 3 -22%', rest: 120 },
      // SÃ©rie 3
      { reps: 'atÃ© falha', loadPct: 0.80, label: 'S3 â€” Carga mÃ¡x.',   rest: 5   },
      { reps: 'atÃ© falha', loadPct: 0.62, label: 'S3 â€” Strip 1 -22%', rest: 5   },
      { reps: 'atÃ© falha', loadPct: 0.48, label: 'S3 â€” Strip 2 -22%', rest: 5   },
      { reps: 'atÃ© falha', loadPct: 0.37, label: 'S3 â€” Strip 3 -22%', rest: 0   },
    ]
  },
  'Rest-Pause': {
    desc: '80-85% 1RM atÃ© a falha, pausa 20s intra-sÃ©rie, pausa 2-3min entre clusters. Repete 2-3 clusters.',
    series: [
      { reps: 'atÃ© falha', loadPct: 0.82, label: 'C1 â€” SÃ©rie',    rest: 20  },
      { reps: 'atÃ© falha', loadPct: 0.82, label: 'C1 â€” Pausa 1',  rest: 20  },
      { reps: 'atÃ© falha', loadPct: 0.82, label: 'C1 â€” Pausa 2',  rest: 150 },
      { reps: 'atÃ© falha', loadPct: 0.82, label: 'C2 â€” SÃ©rie',    rest: 20  },
      { reps: 'atÃ© falha', loadPct: 0.82, label: 'C2 â€” Pausa 1',  rest: 20  },
      { reps: 'atÃ© falha', loadPct: 0.82, label: 'C2 â€” Pausa 2',  rest: 150 },
      { reps: 'atÃ© falha', loadPct: 0.82, label: 'C3 â€” SÃ©rie',    rest: 20  },
      { reps: 'atÃ© falha', loadPct: 0.82, label: 'C3 â€” Pausa 1',  rest: 20  },
      { reps: 'atÃ© falha', loadPct: 0.82, label: 'C3 â€” Pausa 2',  rest: 0   },
    ]
  },
  'Cluster': {
    desc: '88% 1RM Â· 2-3 reps Â· pausa 30s intra-sÃ©rie Â· 5 mini-sÃ©ries por set Â· 3-5 min entre sets. ForÃ§a mÃ¡xima.',
    series: [
      // Set 1 â€” 5 clusters
      { reps: '2-3', loadPct: 0.88, label: 'Set 1 â€” Mini 1', rest: 30 },
      { reps: '2-3', loadPct: 0.88, label: 'Set 1 â€” Mini 2', rest: 30 },
      { reps: '2-3', loadPct: 0.88, label: 'Set 1 â€” Mini 3', rest: 30 },
      { reps: '2-3', loadPct: 0.88, label: 'Set 1 â€” Mini 4', rest: 30 },
      { reps: '2-3', loadPct: 0.88, label: 'Set 1 â€” Mini 5', rest: 210 },
      // Set 2 â€” 5 clusters
      { reps: '2-3', loadPct: 0.88, label: 'Set 2 â€” Mini 1', rest: 30 },
      { reps: '2-3', loadPct: 0.88, label: 'Set 2 â€” Mini 2', rest: 30 },
      { reps: '2-3', loadPct: 0.88, label: 'Set 2 â€” Mini 3', rest: 30 },
      { reps: '2-3', loadPct: 0.88, label: 'Set 2 â€” Mini 4', rest: 30 },
      { reps: '2-3', loadPct: 0.88, label: 'Set 2 â€” Mini 5', rest: 210 },
      // Set 3 â€” 5 clusters
      { reps: '2-3', loadPct: 0.88, label: 'Set 3 â€” Mini 1', rest: 30 },
      { reps: '2-3', loadPct: 0.88, label: 'Set 3 â€” Mini 2', rest: 30 },
      { reps: '2-3', loadPct: 0.88, label: 'Set 3 â€” Mini 3', rest: 30 },
      { reps: '2-3', loadPct: 0.88, label: 'Set 3 â€” Mini 4', rest: 30 },
      { reps: '2-3', loadPct: 0.88, label: 'Set 3 â€” Mini 5', rest: 0  },
    ]
  },
  'FST-7': {
    desc: '7 sÃ©ries do isolador Â· 65% 1RM Â· 30-45s descanso com ALONGAMENTO ATIVO do mÃºsculo-alvo entre as sÃ©ries. Alta congestÃ£o.',
    series: Array.from({length:7}, (_,i) => ({ reps:'12-15', loadPct:0.65, label:`S${i+1} â€” alongar 30s`, rest: i===6 ? 0 : 40 }))
  },

  // â”€â”€ MÃ‰TODOS SEM VARIAÃ‡ÃƒO DE CARGA POR SÃ‰RIE â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  // Usam carga constante â€” loadPct 1.0 em todas as sÃ©ries
  'Unilateral': {
    desc: 'Usar 1RM UNILATERAL (10-20% maior que metade do bilateral). 30s entre lados, 90s entre sÃ©ries completas.',
    series: [
      { reps:'10-12', loadPct:0.72, label:'S1 â€” Lado D', rest:30  },
      { reps:'10-12', loadPct:0.72, label:'S1 â€” Lado E', rest:90  },
      { reps:'10-12', loadPct:0.72, label:'S2 â€” Lado D', rest:30  },
      { reps:'10-12', loadPct:0.72, label:'S2 â€” Lado E', rest:90  },
      { reps:'10-12', loadPct:0.72, label:'S3 â€” Lado D', rest:30  },
      { reps:'10-12', loadPct:0.72, label:'S3 â€” Lado E', rest:0   },
    ]
  },
  'ExcÃªntrico Acentuado': {
    desc: '4-6s na descida Â· 75% 1RM Â· alto TUT Â· maior DOMS. 150-180s descanso (recuperaÃ§Ã£o muscular completa).',
    series: [
      { reps:'6-8', loadPct:0.75, label:'S1 â€” 4s excÃªntrico', rest:150 },
      { reps:'6-8', loadPct:0.75, label:'S2 â€” 4s excÃªntrico', rest:150 },
      { reps:'6-8', loadPct:0.75, label:'S3 â€” 4s excÃªntrico', rest:150 },
      { reps:'6-8', loadPct:0.75, label:'S4 â€” 4s excÃªntrico', rest:0   },
    ]
  },
  'Isometria': {
    desc: 'Definir Ã‚NGULO de execuÃ§Ã£o (ganho especÃ­fico Â±15Â°). Hipertrofia: 65% Ã— 6-10s. ForÃ§a: 80% Ã— 3-6s. 3 reps por posiÃ§Ã£o.',
    series: [
      { reps:'6-10s Ã— 3', loadPct:0.65, label:'S1 â€” Ã‚ngulo A (def. oÂ°)', rest:90 },
      { reps:'6-10s Ã— 3', loadPct:0.65, label:'S2 â€” Ã‚ngulo A',           rest:90 },
      { reps:'6-10s Ã— 3', loadPct:0.65, label:'S3 â€” Ã‚ngulo A',           rest:0  },
    ]
  },
  '21s': {
    desc: 'Ordem: 7 parcial inferior (0Â°-90Â°) â†’ 7 parcial superior (90Â°-180Â°) â†’ 7 completas. ~52% 1RM. 90-120s descanso.',
    series: [
      { reps:'21 (7+7+7)', loadPct:0.52, label:'S1 â€” Infâ†’Supâ†’Completo', rest:100 },
      { reps:'21 (7+7+7)', loadPct:0.52, label:'S2 â€” Infâ†’Supâ†’Completo', rest:100 },
      { reps:'21 (7+7+7)', loadPct:0.52, label:'S3 â€” Infâ†’Supâ†’Completo', rest:0   },
    ]
  },
  'PrÃ©-exaustÃ£o': {
    desc: 'Isolamento (~65%) â†’ Composto (~62% real pÃ³s-fadiga). ATENÃ‡ÃƒO: evidÃªncia cientÃ­fica questionada (Gentil 2007). Use RPE 8 como guia.',
    series: [
  ...
  'Super-sÃ©rie Agonista': {
    desc: 'Mesmo grupo Â· sem pausa entre Ex A e Ex B Â· Ex B perde 10-25% de performance. 90-120s apÃ³s o par.',
    series: [
      { reps:'10-12', loadPct:0.68, label:'Ex A â€” S1', rest:0   },
      { reps:'8-10',  loadPct:0.62, label:'Ex B â€” S1', rest:100 },
      { reps:'10-12', loadPct:0.68, label:'Ex A â€” S2', rest:0   },
      { reps:'8-10',  loadPct:0.62, label:'Ex B â€” S2', rest:100 },
      { reps:'10-12', loadPct:0.68, label:'Ex A â€” S3', rest:0   },
      { reps:'8-10',  loadPct:0.62, label:'Ex B â€” S3', rest:0   },
    ]
  },
  'Super-sÃ©rie Antagonista': {
    desc: 'Grupos opostos alternados sem pausa. Ex: Rosca + TrÃ­ceps. FacilitaÃ§Ã£o recÃ­proca permite manter 70% em ambos.',
    series: [
  ...
  'Bi-set': {
    desc: 'Mesmo mÃºsculo Â· Ex B com carga menor (fadiga acumulada) Â· 90-120s apÃ³s o par. Similiar Ã  super-sÃ©rie agonista.',
    series: [
      { reps:'10-12', loadPct:0.68, label:'Ex A â€” S1', rest:0   },
      { reps:'8-10',  loadPct:0.62, label:'Ex B â€” S1', rest:100 },
      { reps:'10-12', loadPct:0.68, label:'Ex A â€” S2', rest:0   },
  ...
  'Tri-set': {
    desc: '3 exercÃ­cios consecutivos sem descanso. Alto estÃ­mulo metabÃ³lico. 120s apÃ³s cada tri-set completo.',
    series: [
      { reps:'10-12', loadPct:0.68, label:'Ex 1 â€” S1', rest:0   },
      { reps:'10-12', loadPct:0.65, label:'Ex 2 â€” S1', rest:0   },
      { reps:'10-12', loadPct:0.62, label:'Ex 3 â€” S1', rest:120 },
      { reps:'10-12', loadPct:0.68, label:'Ex 1 â€” S2', rest:0   },
      { reps:'10-12', loadPct:0.65, label:'Ex 2 â€” S2', rest:0   },
      { reps:'10-12', loadPct:0.62, label:'Ex 3 â€” S2', rest:120 },
      { reps:'10-12', loadPct:0.68, label:'Ex 1 â€” S3', rest:0   },
      { reps:'10-12', loadPct:0.65, label:'Ex 2 â€” S3', rest:0   },
      { reps:'10-12', loadPct:0.62, label:'Ex 3 â€” S3', rest:0   },
    ]
  },
  'SÃ©rie Gigante': {
    desc: '4 exercÃ­cios seguidos sem pausa Â· 60-65% 1RM Â· 180s descanso apÃ³s cada bloco completo Â· 3 blocos.',
    series: [
      { reps:'10-15', loadPct:0.64, label:'Bloco 1 â€” Ex 1', rest:0   },
      { reps:'10-15', loadPct:0.62, label:'Bloco 1 â€” Ex 2', rest:0   },
      { reps:'10-15', loadPct:0.61, label:'Bloco 1 â€” Ex 3', rest:0   },
      { reps:'10-15', loadPct:0.60, label:'Bloco 1 â€” Ex 4', rest:180 },
      { reps:'10-15', loadPct:0.64, label:'Bloco 2 â€” Ex 1', rest:0   },
      { reps:'10-15', loadPct:0.62, label:'Bloco 2 â€” Ex 2', rest:0   },
      { reps:'10-15', loadPct:0.61, label:'Bloco 2 â€” Ex 3', rest:0   },
      { reps:'10-15', loadPct:0.60, label:'Bloco 2 â€” Ex 4', rest:180 },
      { reps:'10-15', loadPct:0.64, label:'Bloco 3 â€” Ex 1', rest:0   },
      { reps:'10-15', loadPct:0.62, label:'Bloco 3 â€” Ex 2', rest:0   },
      { reps:'10-15', loadPct:0.61, label:'Bloco 3 â€” Ex 3', rest:0   },
      { reps:'10-15', loadPct:0.60, label:'Bloco 3 â€” Ex 4', rest:0   },
    ]
  },
};

// â”€â”€ METADADOS DE CARDIO (FC Alvo, duraÃ§Ã£o, zonas) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// Usado na seleÃ§Ã£o de carga da periodizaÃ§Ã£o para mÃ©todos de cardio
export const METHOD_CARDIO_META = {
  'Zona 1 (Z1)': {
    fcPct:[50,65], durationMin:[20,60], rpe:'1-3',
    note:'RecuperaÃ§Ã£o ativa. Abaixo do VT1. Lactato < 1.5 mmol/L. Conversa completamente fÃ¡cil. FC% Ã© estimativa â€” calibre com teste de campo.'
  },
  'Zona 2 (Z2)': {
    fcPct:[65,75], durationMin:[30,90], rpe:'3-4',
    note:'Base aerÃ³bica. Entre Z1 e VT1. Lactato 1.5-2 mmol/L. DiÃ¡logo em frases completas. AtenÃ§Ã£o: FC% varia Â±10-15 bpm entre indivÃ­duos â€” use teste de campo para calibrar seu Z2 real.'
  },
  'Zona 3 (Z3) â€” Zona Cinzenta': {
    fcPct:[75,87], durationMin:[20,40], rpe:'5-6',
    note:'Zona cinzenta entre VT1 e VT2. Lactato 2-4 mmol/L. DifÃ­cil conversar. Evitar uso excessivo â€” acumula fadiga sem estÃ­mulo de VO2max. Preferir Z2 (base) ou Z4 (limiar).'
  },
  'Zona 4 (Z4) â€” Limiar': {
    fcPct:[85,92], durationMin:[20,40], rpe:'7-8',
    note:'Acima do VT2 / OBLA (lactato > 4 mmol/L). Palavras isoladas. Treino de Tempo Run: mÃ­nimo 20 min para adaptaÃ§Ã£o do tamponamento de lactato. Atletas treinados tÃªm VT2 deslocado para direita.'
  },
  'Zona 5 (Z5) â€” VO2max': {
    fcPct:[90,100], durationMin:[3,8], rpe:'9-10',
    note:'Acima do VT2. Intervalos 3-5 min a 95-100% VO2max (vVO2max). Protocolo Billat: esforÃ§o Ã· 2 de recuperaÃ§Ã£o ativa.'
  },
  'Tabata': {
    fcPct:[90,100], durationMin:[4,12], rpe:'9-10',
    note:'20s all-out / 10s repouso Ã— 8 rounds. Protocolo original: ergÃ´metro de ciclismo a 170% VO2max (Tabata 1996). Em musculaÃ§Ã£o o estÃ­mulo metabÃ³lico Ã© menor. MÃ¡x 2-3Ã—/semana.'
  },
  'HIIT 1:2': {
    fcPct:[85,95], durationMin:[15,25], rpe:'8-9',
    note:'30s esforÃ§o / 60s recuperaÃ§Ã£o (passiva ou caminhada). 6-12 rounds. RazÃ£o 1:2 = maior recuperaÃ§Ã£o entre tiros. MÃ¡x 2-3Ã—/semana.'
  },
  'HIIT 1:1': {
    fcPct:[85,95], durationMin:[10,18], rpe:'8-9',
    note:'30s esforÃ§o / 30s recuperaÃ§Ã£o. Mais desgastante que 1:2 â€” duraÃ§Ã£o total menor (10-18 min). MÃ¡x 2Ã—/semana. Risco de overreaching se usado frequentemente sem periodizaÃ§Ã£o.'
  },
  'SIT (Sprint Interval Training)': {
    fcPct:[95,100], durationMin:[8,20], rpe:'10',
    note:'EsforÃ§o ALL-OUT absoluto â€” nÃ£o regulado por FC. Protocolo Wingate: 30s mÃ¡ximos no ergÃ´metro Ã— 4-6 sprints com 4 min recuperaÃ§Ã£o passiva. Para corrida: 6-10s sprints com 2-3 min recuperaÃ§Ã£o.'
  },
  'SÃ©rie de RepetiÃ§Ã£o (VO2max)': {
    fcPct:[90,100], durationMin:[15,30], rpe:'9-10',
    note:'Intervalos de 3-5 min a 95-100% VO2max com recuperaÃ§Ã£o ativa igual ao esforÃ§o (razÃ£o 1:1 em tempo). Protocolo Billat. 4-6 repetiÃ§Ãµes por sessÃ£o.'
  },
  'Steady State Z2': {
    fcPct:[65,75], durationMin:[20,60], rpe:'3-4',
    note:'Ritmo constante em Z2. Base aerÃ³bica, oxidaÃ§Ã£o de gordura, mitocÃ´ndrias. MÃ­nimo 30 min para adaptaÃ§Ã£o. Conversa em frases completas.'
  },
  'Progressivo': {
    fcPct:[60,90], durationMin:[20,60], rpe:'3-8',
    note:'+0.5 km/h a cada 5 min (corrida) ou +10-15W a cada 3 min (ciclismo). Cobre Z2 â†’ Z4. Pode ser usado como teste de limiar (protocolo Conconi).'
  },
};
  ...
  switch (methodName) {
    case 'Unilateral': {
      for (let si = 0; si < n; si++) {
        ...
        list.push({
          reps: '10-12',
          loadPct: 0.72,
          label: `S${setNum} â€” Lado ${isEsq ? 'E' : 'D'}`,
    ...
    case 'Drop-set': {
      for (let si = 0; si < n; si++) {
        ...
        const type = si % 3;
        let label = `S${setNum} â€” Principal`;
        let loadPct = 0.75;
        let rest = 5;
        if (type === 1) {
          label = `S${setNum} â€” Drop 1 -20%`;
          loadPct = 0.60;
          rest = 5;
        } else if (type === 2) {
          label = `S${setNum} â€” Drop 2 -20%`;
          loadPct = 0.48;
          rest = si === n - 1 ? 0 : 120;
        }
        list.push({ reps: 'atÃ© falha', loadPct, label, rest });
      }
      return list;
    }
    case 'Stripping': {
      for (let si = 0; si < n; si++) {
        ...
        const type = si % 4;
        let label = `S${setNum} â€” Carga mÃ¡x.`;
        let loadPct = 0.80;
        let rest = 5;
        if (type === 1) {
          label = `S${setNum} â€” Strip 1 -22%`;
          loadPct = 0.62;
        } else if (type === 2) {
          label = `S${setNum} â€” Strip 2 -22%`;
          loadPct = 0.48;
        } else if (type === 3) {
          label = `S${setNum} â€” Strip 3 -22%`;
          loadPct = 0.37;
          rest = si === n - 1 ? 0 : 120;
        }
        list.push({ reps: 'atÃ© falha', loadPct, label, rest });
    ...
    case 'Rest-Pause': {
      for (let si = 0; si < n; si++) {
        const setNum = Math.floor(si / 3) + 1;
        const type = si % 3;
        let label = `C${setNum} â€” SÃ©rie`;
        let rest = 20;
        if (type === 1) {
          label = `C${setNum} â€” Pausa 1`;
        } else if (type === 2) {
          label = `C${setNum} â€” Pausa 2`;
          rest = si === n - 1 ? 0 : 150;
        }
        list.push({ reps: 'atÃ© falha', loadPct: 0.82, label, rest });
      }
      return list;
    }
    case 'Cluster': {
      for (let si = 0; si < n; si++) {
        ...
        const type = si % 5;
        const label = `Set ${setNum} â€” Mini ${type + 1}`;
    ...
    case 'FST-7': {
      for (let si = 0; si < n; si++) {
        list.push({
          reps: '12-15',
          loadPct: 0.65,
          label: `S${si + 1} â€” alongar 30s`,
          rest: si === n - 1 ? 0 : 40
        });
      }
      return list;
    }
    case 'ExcÃªntrico Acentuado': {
      for (let si = 0; si < n; si++) {
        list.push({
          reps: '6-8',
          loadPct: 0.75,
          label: `S${si + 1} â€” 4s excÃªntrico`,
          rest: si === n - 1 ? 0 : 150
        });
      }
      return list;
    ...
    case '21s': {
      for (let si = 0; si < n; si++) {
        list.push({
          reps: '21 (7+7+7)',
          loadPct: 0.52,
          label: `S${si + 1} â€” Infâ†’Supâ†’Completo`,
          rest: si === n - 1 ? 0 : 100
        });
      }
      return list;
    }
    case 'PirÃ¢mide Crescente': {
      for (let si = 0; si < n; si++) {
    ...
    case 'PirÃ¢mide Decrescente': {
      for (let si = 0; si < n; si++) {
        ...
        const labels = ['MÃ¡ximo', 'Pesada', 'Moderada', 'Leve/MÃ¡x'];
        const lbl = si === 0 ? 'MÃ¡ximo' : si === n - 1 ? 'Leve/MÃ¡x' : labels[Math.min(labels.length - 2, si)] || 'Moderada';
        list.push({
          reps,
          loadPct: parseFloat(pct.toFixed(2)),
          label: `S${si + 1} â€” ${lbl}`,
    ...
    case 'PirÃ¢mide Dupla': {
      for (let si = 0; si < n; si++) {
        ...
        list.push({
          reps,
          loadPct: parseFloat(pct.toFixed(2)),
          label: `S${si + 1} â€” ${si === half ? 'Pico â†‘' : si < half ? 'Crescente' : 'Decrescente'}`,
          rest
        });
      }
      return list;
    }
    case 'PirÃ¢mide Completa': {
      for (let si = 0; si < n; si++) {
        const half = Math.floor(n / 2);
        let pct;
        if (si <= half) {
...
export function rebuildMethodSeriesPanel(row, index, preserveExisting = true) {
  ...
  const seriesHTML = dynamicSeries.map((s, si) => {
    ...
    return `
      ${isNewCluster ? `<div style="grid-column:1/-1;height:1px;background:rgba(245,158,11,0.2);margin:3px 0" title="PrÃ³ximo cluster â€” 2-3min descanso"></div>` : ''}
      <div style="display:grid;grid-template-columns:100px 1fr 72px 72px 56px;gap:6px;align-items:center;padding:5px 0;border-bottom:1px solid rgba(148,163,184,0.08)" data-serie="${si}">
        ...
        <div title="${isClusterMethod && curLabel.toLowerCase().includes('pausa') ? 'Pausa intra-sÃ©rie (20s). Entre clusters: 2-3min.' : ''}">
  ...
  const seriesLegend = `
    <div style="display:grid;grid-template-columns:100px 1fr 72px 72px 56px;gap:6px;margin-bottom:4px">
      <div style="font-size:0.6rem;color:var(--text-muted);text-transform:uppercase">${isClusterMethod ? 'Mini-sÃ©rie' : 'SÃ©rie'}</div>
  ...
  // Sincronizar carga base se jÃ¡ preenchida
...
function exerciseRowHTML(index, ex = {}, allExercises = [], allMethods = []) {
  ...
  if (progression && !isCombined) {
    // MÃ©todos NÃƒO combinados: mostrar painel de sÃ©ries com % 1RM normalmente
    ...
    const seriesHTML = dynamicSeries.map((s, si) => {
      ...
      // Para linhas extras (clusters 2 e 3 sem save), propagar a carga da sÃ©rie 0
    ...
    // MÃ©todos combinados: banner de pareamento â€” sem painel de sÃ©ries individual
    const COMBINED_LABELS = {
      'Bi-set':                  'Execute este exercÃ­cio imediatamente em sequÃªncia com o prÃ³ximo da lista. Descanse apenas apÃ³s completar o par.',
      'Super-sÃ©rie Agonista':    'Mesmo grupo muscular, sem pausa entre os dois. Descanse apÃ³s o segundo exercÃ­cio do par.',
      'Super-sÃ©rie Antagonista': 'Grupos opostos (ex: BÃ­ceps â†’ TrÃ­ceps) sem pausa. Descanse apÃ³s o segundo.',
      'Tri-set':                 '3 exercÃ­cios consecutivos sem pausa. Descanse apenas apÃ³s o terceiro.',
      'SÃ©rie Gigante':           '4+ exercÃ­cios sem pausa, cargas reduzidas (~60%). Descanse apÃ³s o Ãºltimo do grupo.',
      'PrÃ©-exaustÃ£o':            'Isolamento executado antes do composto, sem pausa. O isolamento fatiga o mÃºsculo-alvo primeiro.',
    };
    const desc = COMBINED_LABELS[ex.method] || `Execute em sequÃªncia com o exercÃ­cio adjacente. Descanse apenas apÃ³s o grupo completo.`;
    methodPanelHTML = `
      <div style="grid-column:1/-1;margin-top:4px;padding:8px 10px;background:rgba(245,158,11,0.07);border:1px solid rgba(245,158,11,0.25);border-radius:8px">
        <div style="display:flex;align-items:center;gap:6px;margin-bottom:3px">
          ...
          <span style="font-size:0.65rem;color:var(--text-muted);background:rgba(245,158,11,0.12);padding:1px 6px;border-radius:8px">Descanso pÃ³s-par: ${ex.rest || 90}s</span>
        </div>
        ...
        <div style="font-size:0.65rem;color:var(--accent);margin-top:4px">
          Certifique-se de que o exercÃ­cio parceiro tambÃ©m estÃ¡ marcado com o mesmo mÃ©todo e logo abaixo na lista.
        </div>
      </div>`;
  } else if (ex.method) {
    ...
    if (desc) {
      methodPanelHTML = `
        <div class="method-tip" style="font-size:0.72rem;color:var(--accent);margin-top:4px;grid-column:1/-1;padding:6px 8px;background:rgba(6,182,212,0.07);border-radius:6px;border-left:2px solid var(--accent)">
          <strong>${ex.method}</strong> â€” ${desc}
        </div>`;
    }
  }
  ...
  return `
    <div class="exercise-row" style="
      ...
      <div>
        <label class="form-label" style="font-size:0.65rem;margin-bottom:2px;opacity:0.65">ExercÃ­cio</label>
      ...
      <div>
        <label class="form-label" style="font-size:0.65rem;margin-bottom:2px;opacity:0.65">SÃ©ries</label>
      ...
      <div>
        <label class="form-label" style="font-size:0.65rem;margin-bottom:2px;opacity:0.65">MÃ©todo</label>
      ...
      <button type="button" class="btn btn-ghost btn-icon view-cardio-chart" data-index="${index}"
        style="color:var(--primary);padding:4px;align-self:flex-end;margin-bottom:2px;visibility:${isTime ? 'visible' : 'hidden'}" title="Ver GrÃ¡fico de Ritmo">
      ...
      ${methodPanelHTML}
      <!-- ObservaÃ§Ãµes do personal para este exercÃ­cio -->
      <div style="grid-column:1/-1;margin-top:4px">
        <input class="form-input ex-notes-input" name="ex_notes_${index}"
          value="${ex.trainerNotes || ex.notes || ''}"
          placeholder="ðŸ“ OrientaÃ§Ãµes tÃ©cnicas para o aluno (ex: manter core ativado, cotovelo para dentro...)"
          style="font-size:0.75rem;color:var(--text-muted);background:rgba(16,185,129,0.03);border-color:rgba(16,185,129,0.15)" />
      </div>
    </div>`;
}

// MÃ©todos combinados â€” descanso Ã© compartilhado pÃ³s-Ãºltimo exercÃ­cio do par
export const COMBINED_METHODS = new Set([
  'Bi-set','Super-sÃ©rie Agonista','Super-sÃ©rie Antagonista',
  'Tri-set','SÃ©rie Gigante','PrÃ©-exaustÃ£o'
]);
...
function collectExercises() {
  ...
  rows.forEach(row => {
    ...
    if (seriesPanel && METHOD_PROGRESSIONS[method]) {
      ...
      exercises.push({
        ...
        reps:              serieLogs.map(s=>s.reps).join('â†’'),
  ...
  // Atribuir groupId compartilhado para exercÃ­cios consecutivos com mesmo mÃ©todo combinado
  let groupCounter = 0;
  for (let i = 0; i < exercises.length; i++) {
    const ex = exercises[i];
    if (!COMBINED_METHODS.has(ex.method || '')) continue;
    if (ex.groupId) continue; // jÃ¡ atribuÃ­do
    // Agrupar todos os consecutivos com mesmo mÃ©todo
    ...
    for (let j = i + 1; j < exercises.length; j++) {
      if (exercises[j].method === ex.method) {
        ...
        break; // para na primeira quebra de sequÃªncia
      }
    }
  }
...
export function initWorkouts(navigateFn) {
  const openAddModal = async () => {
    ...
    openModal({
      ...
      actions: [
        ...
        { label: 'Salvar Treino', class: 'btn-primary', onClick: async () => {
          ...
          if (!data.studentId || !data.name) { notify.error('Aluno e nome sÃ£o obrigatÃ³rios'); return; }
    ...
    // Substituir primeira linha com mÃ©todos
    setTimeout(() => {
      ...
      openModal({
        ...
        content: `
          ...
          <div class="table-container mb-lg">
            <table class="data-table">
              <thead><tr><th>#</th><th>ExercÃ­cio</th><th>SÃ©ries</th><th>Reps</th><th>Carga</th><th>Desc.</th><th>MÃ©todo</th><th>Tipo</th></tr></thead>
              <tbody>
                ${(w.exercises||[]).map((e, i) => {
          ...
          ${doneSessions.length ? `
            <div style="border-top:1px solid var(--border-color);padding-top:16px;margin-top:16px">
              <h4 class="mb-sm" style="color:var(--success)">ðŸ“Š HistÃ³rico de RealizaÃ§Ãµes</h4>
                ...
                ${doneSessions.map((se, si) => {
                  ...
                  <div style="background:var(--bg-page);border:1px solid var(--border-color);border-radius:10px;overflow:hidden">
                    ...
                    <div style="padding:10px 14px;border-bottom:1px solid var(--border-color);display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:6px">
                      ...
                      <span style="font-size:0.75rem;color:var(--text-muted)">${Calc.formatDate(se.date || se.createdAt)} Â· Volume: <strong style="color:var(--primary)">${se.totalVolume || 0}kg</strong></span>
                    </div>
                    ...
                    <div style="padding:8px 14px;background:rgba(255,255,255,0.02);border-bottom:1px solid var(--border-color);display:flex;flex-wrap:wrap;gap:12px;align-items:center">
                      <span style="font-size:0.7rem;color:var(--text-muted);font-weight:600;text-transform:uppercase;letter-spacing:0.05em">Check-in PÃ³s</span>
                    ...
                    <div style="padding:10px 14px">
                      ...
                      <div style="display:flex;gap:12px;margin-bottom:8px;flex-wrap:wrap">
                        <span style="font-size:0.62rem;color:var(--warning)">â–  PSE â€” esforÃ§o percebido</span>
                        <span style="font-size:0.62rem;color:var(--accent)">â–  RIR â€” reps no tanque</span>
                        <span style="font-size:0.62rem;color:var(--text-muted)">â–  1RM â€” estimativa Epley</span>
                      </div>
                      ...
                      <table style="width:100%;border-collapse:collapse;font-size:0.75rem">
                        <thead>
                          <tr style="border-bottom:1px solid var(--border-color);color:var(--text-muted);font-size:0.65rem;text-transform:uppercase;letter-spacing:0.06em">
                            <th style="padding:5px 6px;text-align:left;font-weight:600">ExercÃ­cio</th>
                            <th style="padding:5px 6px;text-align:center;font-weight:600">SÃ©ries</th>
                            <th style="padding:5px 6px;text-align:center;font-weight:600">Reps</th>
                            <th style="padding:5px 6px;text-align:center;font-weight:600">Carga MÃ¡x</th>
                        ...
                        <tbody>
                          ${(w.exercises||[]).map(ex => {
                            ...
                            // 1RM Epley: w Ã— (1 + reps/30) â€” usar sÃ©rie com maior carga
                              ...
                              <div style="margin-top:5px;display:flex;flex-direction:column;gap:2px">
                                ${exSets.map((s, idx) => {
                                  ...
                                  return `<span style="font-size:0.67rem;color:var(--text-muted)">
                                    Â· <strong style="color:var(--text-primary)">S${s.setIdx != null ? s.setIdx + 1 : idx + 1}:</strong>
                                    <strong>${s.reps || 0}Ã—${s.load || 0}kg</strong>
                            ...
                            return `<tr style="border-bottom:1px solid rgba(255,255,255,0.04)">
                              ...
                              <td style="padding:7px 6px;text-align:center;vertical-align:top">${realSets || 'â€”'}</td>
                              <td style="padding:7px 6px;text-align:center;vertical-align:top">${totalReps || 'â€”'}</td>
                              <td style="padding:7px 6px;text-align:center;vertical-align:top;font-weight:700;color:var(--primary)">${maxLoad ? maxLoad + 'kg' : 'â€”'}</td>
                              <td style="padding:7px 6px;text-align:center;vertical-align:top;color:var(--success);font-weight:600">${totalVol ? totalVol + 'kg' : 'â€”'}</td>
                              <td style="padding:7px 6px;text-align:center;vertical-align:top;color:var(--warning);font-weight:700">${avgPse ? avgPse.toFixed(1) : 'â€”'}</td>
                              <td style="padding:7px 6px;text-align:center;vertical-align:top;color:var(--accent);font-weight:700">${(avgRir || avgRir === 0) && exSets.some(s => s.rir != null) ? avgRir.toFixed(1) : 'â€”'}</td>
                              <td style="padding:7px 6px;text-align:center;vertical-align:top;color:var(--text-muted);font-weight:600">${oneRM ? oneRM + 'kg' : 'â€”'}</td>
                            </tr>`;
                          }).join('')}
                        </tbody>
                      </table>
            ...
            </div>` : `<div style="border-top:1px solid var(--border-color);padding-top:12px;margin-top:12px"><p class="text-xs text-muted">Nenhuma sessÃ£o executada deste treino ainda. Inicie o Treino ao Vivo para registrar as cargas reais praticadas.</p></div>`}
        `
      });
    });
  });
  ...
  document.querySelectorAll('.delete-workout').forEach(btn => {
    btn.addEventListener('click', async () => {
      if (window.confirm('Excluir este treino?')) {
        ...
        notify.success('Treino excluÃ­do');
        navigateFn('/treinos');
      }
    });
  });
  ...
  document.querySelectorAll('.edit-workout').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      const students   = await db.getAll('students');
      const allEx      = await db.getAll('exercises');
      const allMethods = await db.getAll('methods');   // â† carrega mÃ©todos
        ...
        const exDetails = (lastSession.exercises || []).map((e, i) => {
          ...
          if (!sets.length) return `
            <div style="background:var(--bg-card);border:1px solid var(--border-color);border-radius:8px;padding:10px 12px;opacity:0.5">
              ...
              <div style="font-size:0.7rem;color:var(--text-muted);margin-top:4px">NÃ£o realizado</div>
            </div>`;
          ...
          const setsHTML = sets.map(s => {
            ...
            return `<div style="display:flex;align-items:center;gap:6px;padding:3px 0;border-bottom:1px solid rgba(148,163,184,0.08);font-size:0.72rem">
              ...
              <span style="font-weight:700;color:var(--text-primary)">${s.reps || 0} Ã— ${s.load || 0}kg</span>
            ...
            <div style="background:var(--bg-card);border:1px solid var(--border-color);border-radius:8px;padding:10px 12px">
              <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
                ...
                <div style="display:flex;gap:8px;font-size:0.7rem;color:var(--text-muted)">
                  <span>${sets.length} sÃ©r.</span>
                  <span>${totalReps} reps</span>
                  <span style="color:var(--primary);font-weight:600">MÃ¡x: ${maxLoad}kg</span>
          ...
          <div style="background:rgba(16,185,129,0.06);border:1px solid rgba(16,185,129,0.2);border-radius:10px;margin-bottom:16px;overflow:hidden">
            <div style="display:flex;justify-content:space-between;align-items:center;padding:12px 14px;cursor:pointer;user-select:none" onclick="const d=this.nextElementSibling;d.style.display=d.style.display==='none'?'block':'none';this.querySelector('.ls-chevron').style.transform=d.style.display==='none'?'':'rotate(180deg)'">
              <div style="display:flex;align-items:center;gap:10px">
                <span style="font-size:0.85rem;font-weight:700;color:var(--success)">ðŸ“Š Ãšltimo treino: ${new Date(lastSession.date).toLocaleDateString('pt-BR', { weekday:'short', day:'numeric', month:'short' })}</span>
                <div style="display:flex;gap:8px;flex-wrap:wrap">
                  <span style="font-size:0.72rem;background:var(--bg-card);padding:2px 8px;border-radius:10px;border:1px solid var(--border-color)">â± ${durMin}min</span>
                  <span style="font-size:0.72rem;background:var(--bg-card);padding:2px 8px;border-radius:10px;border:1px solid var(--border-color)">ðŸ‹ï¸ ${lastSession.totalVolume || 0}kg</span>
                  <span style="font-size:0.72rem;background:var(--bg-card);padding:2px 8px;border-radius:10px;border:1px solid var(--border-color)">ðŸ“Š ${lastSession.totalSets || 0} sÃ©ries</span>
          ...
          { label: 'Salvar', class: 'btn-primary', onClick: async (e) => {
            ...
            try {
              ...
              if (!data.studentId || !data.name) {
                notify.error('Aluno e nome sÃ£o obrigatÃ³rios');
...
function bindRemoveExercise() {
  document.querySelectorAll('.remove-exercise').forEach(btn => {
    btn.onclick = () => {
      ...
      // Remover tambÃ©m o conector logo apÃ³s a linha
...
function bindExerciseRowHandlers(allExercises, allMethods) {
  ...
  // â”€â”€ Auto-preenchimento ao selecionar MÃ‰TODO â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  document.querySelectorAll('.ex-method').forEach(sel => {
    sel.onchange = () => {
      ...
      // Remover painel de sub-sÃ©ries anterior
      row?.querySelectorAll('.method-series-panel').forEach(p => p.remove());
      row?.querySelectorAll('.method-tip').forEach(p => p.remove());

      if (!methodName) {
        // Limpar indicaÃ§Ã£o de mÃ©todo
      ...
      // Preencher sÃ©ries/reps/descanso padrÃ£o
      const setsEl = row?.querySelector(`[name="ex_sets_${i}"]`);
      const repsEl = row?.querySelector(`[name="ex_reps_${i}"]`);
      const restEl = row?.querySelector(`[name="ex_rest_${i}"]`);
      ...
      // â”€â”€ MÃ‰TODO COMBINADO: banner + auto-adicionar exercÃ­cio par â”€â”€
      if (isCombinedMethod) {
        row?.querySelectorAll('.method-series-panel,.method-tip,.combined-banner').forEach(p => p.remove());
        if (restEl) restEl.value = '0';

        const COMBINED_DESC = {
          'Bi-set': 'Execute com o prÃ³ximo exercÃ­cio sem descanso. Descanse apenas apÃ³s completar o par.',
          'Super-sÃ©rie Agonista': 'Mesmo grupo muscular em sequÃªncia sem pausa. Descanse apÃ³s o par.',
          'Super-sÃ©rie Antagonista': 'Grupos opostos (ex: BÃ­ceps â†’ TrÃ­ceps) sem pausa. Descanse apÃ³s o par.',
          'Tri-set': '3 exercÃ­cios consecutivos sem pausa. Descanse apÃ³s o terceiro.',
          'SÃ©rie Gigante': '4+ exercÃ­cios consecutivos sem pausa. Cargas reduzidas ~60%. Descanse apÃ³s o Ãºltimo.',
          'PrÃ©-exaustÃ£o': 'Isolamento â†’ Composto sem pausa. Fatiga o mÃºsculo-alvo primeiro.',
        };
  ...
  // â”€â”€ Atualizar painel ao mudar nÃºmero de SÃ‰RIES â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ Auto-preencher tipo de carga ao selecionar exercÃ­cio â”€â”€â”€â”€
  ...
  // â”€â”€ Atualizar label ao mudar tipo de carga â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ Sincronizar painÃ©is de mÃ©todos prÃ©-existentes (na ediÃ§Ã£o) â”€â”€
  ...
  // â”€â”€ Ritmo / Cardio chart preview â”€â”€
  bindCardioChartPreview();
}

// â”€â”€ Atualizar visual de pares combinados na lista de exercÃ­cios â”€â”€
function refreshCombinedVisuals() {
  ...
  rows.forEach((row, idx) => {
    ...
    // Conector "â†’ sem descanso" entre exercÃ­cios do grupo
    if (!isLast) {
      ...
      connector.innerHTML = `
        ...
        sem descanso â†’ continuar`;
      row.insertAdjacentElement('afterend', connector);
    }
  });
}

// â”€â”€ CARDIO GRAPH HELPERS & MODAL FOR TRAINER â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function showCardioPreviewModal(ex) {
  ...
  const content = `
    <div style="background:rgba(255,255,255,0.02);border-radius:12px;padding:12px;border:1px solid rgba(255,255,255,0.06);margin-bottom:12px">
      <div style="font-size:0.75rem;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;color:var(--primary);margin-bottom:10px;display:flex;align-items:center;justify-content:space-between">
        ...
        <span style="font-size:0.68rem;color:var(--text-secondary);font-weight:500;text-transform:none;letter-spacing:0">DuraÃ§Ã£o: ${formatDurationText(totalSec)}</span>
      </div>
          ...
          return `
            <div style="display:flex;align-items:center;justify-content:space-between;padding:6px 10px;background:rgba(255,255,255,0.015);border-radius:8px;font-size:0.75rem;border-left:3px solid ${getZoneColor(seg.intensity)}">
              <div style="display:flex;flex-direction:column;text-align:left">
                ...
                <span style="font-size:0.65rem;color:var(--text-muted,#94a3b8)">â± ${timeLabel} (${formatTimeLabel(seg.duration)})</span>
              </div>
  ...
  openModal({
    title: `VisualizaÃ§Ã£o de Ritmo â€” ${ex.name || 'Cardio'}`,
    content: content,
    size: 'md',
