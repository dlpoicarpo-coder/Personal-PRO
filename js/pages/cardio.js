...
const CARDIO_METHODS = [
  { id: 'mict', name: 'MICT â€” ContÃ­nuo Moderado', desc: 'ExercÃ­cio contÃ­nuo em intensidade moderada (Zona 2-3). Ideal para base aerÃ³bia.', zone: '2-3', duration: '30-60 min', icon: 'â€”' },
  { id: 'hiit', name: 'HIIT â€” Intervalado de Alta Intensidade', desc: 'Intervalos de 30s-4min a 80-95% FCmax com recuperaÃ§Ã£o ativa. Melhora VO2max.', zone: '4-5', duration: '20-40 min', icon: 'â€”' },
  { id: 'sit', name: 'SIT â€” Sprint Interval Training', desc: 'Sprints all-out de 10-30s com recuperaÃ§Ã£o completa (2-4 min). MÃ¡xima potÃªncia anaerÃ³bia.', zone: '5', duration: '15-25 min', icon: 'â€”' },
  { id: 'polarized', name: 'Polarizado', desc: '80% do treino em baixa intensidade (Z1-2) e 20% em alta intensidade (Z4-5). Evita zona 3.', zone: '1-2 + 4-5', duration: 'VariÃ¡vel', icon: 'â€”' },
  { id: 'fartlek', name: 'Fartlek', desc: 'VariaÃ§Ã£o livre de intensidade durante o treino. Combina sprints curtos com recuperaÃ§Ã£o.', zone: '2-5', duration: '20-45 min', icon: 'â€”' },
  { id: 'tabata', name: 'Tabata', desc: 'Protocolo 20s esforÃ§o mÃ¡ximo + 10s descanso Ã— 8 rounds = 4 min. Ultra-eficiente.', zone: '5', duration: '4-16 min', icon: 'â€”' },
  { id: 'threshold', name: 'Limiar AnaerÃ³bio', desc: 'EsforÃ§o sustentado no limiar (Zona 4). Melhora tolerÃ¢ncia ao lactato.', zone: '4', duration: '20-40 min', icon: 'â€”' },
  { id: 'recovery', name: 'RecuperaÃ§Ã£o Ativa', desc: 'ExercÃ­cio muito leve (Zona 1) para promover recuperaÃ§Ã£o entre sessÃµes intensas.', zone: '1', duration: '20-40 min', icon: 'â€”' },
];

const ZONE_COLORS = ['#94a3b8', '#3b82f6', '#10b981', '#f59e0b', '#ef4444'];
const ZONE_NAMES = ['RecuperaÃ§Ã£o', 'Base AerÃ³bia', 'AerÃ³bia', 'Limiar AnaerÃ³bio', 'VO2 Max'];
...
export async function renderCardio() {
  ...
  return `
    <div class="page-header">
      ...
      <button class="btn btn-primary" id="newCardioBtn">+ Nova SessÃ£o Cardio</button>
    </div>

    <div class="tabs" id="cardioTabs">
      <button class="tab active" data-tab="zones">Zonas de Treino</button>
      <button class="tab" data-tab="methods">MÃ©todos</button>
      <button class="tab" data-tab="sessions">SessÃµes Cardio</button>
    </div>
    ...
    <div id="tabZones" class="cardio-tab">
      <div class="card mb-lg">
        ...
        <div class="form-row">
          ...
          <div class="form-group"><label class="form-label">FC MÃ¡xima (bpm)</label>
            <input class="form-input" id="zoneFcMax" type="number" placeholder="Auto: 220 - idade" />
            <div class="form-hint">Deixe vazio para usar fÃ³rmula de Tanaka</div>
          </div>
    ...
    <div id="tabMethods" class="cardio-tab" style="display:none">
      ...
      <div class="card mt-lg">
        <div class="card-header"><span class="card-title">DistribuiÃ§Ã£o Semanal Recomendada</span></div>
        <div class="table-container"><table class="data-table"><thead><tr>
          <th>Objetivo</th><th>MICT (Z2)</th><th>Limiar (Z4)</th><th>HIIT/SIT (Z5)</th><th>RecuperaÃ§Ã£o (Z1)</th><th>Total/Sem</th>
        </tr></thead><tbody>
          <tr><td><strong>SaÃºde Geral</strong></td><td>3Ã—30min</td><td>â€”</td><td>1Ã—20min</td><td>1Ã—20min</td><td>3-5 sessÃµes</td></tr>
          <tr><td><strong>Emagrecimento</strong></td><td>3Ã—40min</td><td>1Ã—25min</td><td>1Ã—20min</td><td>1Ã—20min</td><td>4-6 sessÃµes</td></tr>
          <tr><td><strong>Performance</strong></td><td>2Ã—45min</td><td>1Ã—30min</td><td>2Ã—25min</td><td>1Ã—25min</td><td>5-6 sessÃµes</td></tr>
          <tr><td><strong>Polarizado</strong></td><td>4Ã—40min</td><td>â€”</td><td>1Ã—30min</td><td>â€”</td><td>4-5 sessÃµes</td></tr>
        </tbody></table></div>
      </div>
    </div>
    ...
    <div id="tabSessions" class="cardio-tab" style="display:none">
      ${cardioSessions.length ? `
        <div class="table-container"><table class="data-table"><thead><tr>
          <th>Aluno</th><th>MÃ©todo</th><th>Data</th><th>DuraÃ§Ã£o</th><th>FC MÃ©d</th><th>Zona MÃ©d</th><th>Intensidade (%)</th><th>Velocidade/PotÃªncia</th>
        </tr></thead><tbody>
      ...
      ` : `<div class="empty-state"><div class="empty-icon" style="font-size:2rem">â€”</div><h3>Nenhuma sessÃ£o cardio</h3><p>Crie sessÃµes de treino cardiovascular para seus alunos</p></div>`}
    </div>
  `;
}
  ...
  return `
    <div class="card mb-lg" style="border-top:3px solid var(--primary)">
      <div class="card-header">
        <span class="card-title">Zonas de Treino â€” ${studentName}</span>
        <div class="text-sm text-muted">FCmax: ${fcMax} bpm Â· FCrep: ${fcRep} bpm Â· Reserva: ${fcMax - fcRep} bpm</div>
      </div>
      <div class="zones-visual mb-lg">
        ${zones.map(z => `
          <div class="zone-bar" style="--zone-color:${z.color}">
            ...
            <div class="zone-range">${z.fcMin} â€” ${z.fcMax} bpm</div>
      ...
      <div class="table-container"><table class="data-table"><thead><tr>
        <th>Zona</th><th>Nome</th><th>% FC Reserva</th><th>FC Min</th><th>FC Max</th><th>BenefÃ­cio Principal</th>
      </tr></thead><tbody>
        ${zones.map(z => `<tr>
          ...
          <td>${z.fcMin} bpm</td>
          <td>${z.fcMax} bpm</td>
          <td class="text-sm text-muted">${['RecuperaÃ§Ã£o, aquecimento', 'Base aerÃ³bia, queima de gordura', 'ResistÃªncia aerÃ³bia', 'Limiar anaerÃ³bio, tolerÃ¢ncia ao lactato', 'VO2max, pico de performance'][z.zone - 1]}</td>
        </tr>`).join('')}
      </tbody></table></div>
...
export function initCardio(navigateFn) {
  ...
  document.getElementById('newCardioBtn')?.addEventListener('click', async () => {
    ...
    openModal({
      title: '+ Nova SessÃ£o Cardio', size: 'lg',
      ...
      content: `<form id="cardioForm">
        <div class="form-row">
          ...
          <div class="form-group"><label class="form-label">MÃ©todo *</label>
        ...
        <div class="form-row">
          ...
          <div class="form-group"><label class="form-label">DuraÃ§Ã£o (min)</label>
          ...
          <div class="form-group"><label class="form-label">Modalidade</label>
            <select class="form-select" name="modality"><option>Esteira</option><option>Bicicleta</option><option>ElÃ­ptico</option><option>Remo</option><option>Corrida Ao Ar Livre</option><option>NataÃ§Ã£o</option><option>Pular Corda</option><option>Outro</option></select>
          </div>
        </div>
        <div id="protocolDetails"></div>
        <div style="border-top:1px solid var(--border-color);padding-top:16px;margin-top:16px">
          <h4 class="mb-sm">Dados da SessÃ£o (opcional)</h4>
          <div class="form-row">
            <div class="form-group"><label class="form-label">FC MÃ©dia (bpm)</label><input class="form-input" name="avgHR" type="number" placeholder="Ex: 145" /></div>
            ...
            <div class="form-group"><label class="form-label">Velocidade/PotÃªncia/Ritmo</label><input class="form-input" name="cardioSpeed" placeholder="Ex: 10 km/h ou 5:30/km" /></div>
          ...
          <div class="form-row">
            ...
            <div class="form-group"><label class="form-label">DistÃ¢ncia (km)</label><input class="form-input" name="distance" type="number" step="0.1" placeholder="Ex: 5.2" /></div>
            <div class="form-group"><label class="form-label">Notas</label><input class="form-input" name="notes" placeholder="ObservaÃ§Ãµes..." /></div>
          </div>
        </div>
      </form>`,
      actions: [
        { label: 'Cancelar', class: 'btn-secondary', id: 'cancelCardio', onClick: () => closeModal() },
        { label: 'Salvar SessÃ£o', class: 'btn-primary', id: 'saveCardio', onClick: async () => {
          ...
          notify.success('SessÃ£o cardio registrada!');
          closeModal();
          navigateFn('/cardio');
        }}
      ]
      ...
      document.querySelector('[name="cardioMethod"]')?.addEventListener('change', (e) => {
        ...
        if (e.target.value === 'hiit') {
          html = `<div class="card mt-md" style="background:rgba(239,68,68,0.05);border-color:rgba(239,68,68,0.2)">
            ...
            <div class="form-row">
              <div class="form-group"><label class="form-label">Tempo de EsforÃ§o (s)</label><input class="form-input" name="hiit_work" type="number" value="30" /></div>
              <div class="form-group"><label class="form-label">Tempo de RecuperaÃ§Ã£o (s)</label><input class="form-input" name="hiit_rest" type="number" value="30" /></div>
              ...
              <div class="form-group"><label class="form-label">SÃ©ries</label><input class="form-input" name="hiit_sets" type="number" value="3" /></div>
            </div>
          </div>`;
        } else if (e.target.value === 'tabata') {
          html = `<div class="card mt-md" style="background:rgba(239,68,68,0.05);border-color:rgba(239,68,68,0.2)">
            <h4 class="mb-sm" style="color:var(--danger)">Protocolo Tabata (20s/10s Ã— 8)</h4>
            <div class="form-row">
              <div class="form-group"><label class="form-label">Rounds por sÃ©rie</label><input class="form-input" name="tabata_rounds" type="number" value="8" /></div>
              <div class="form-group"><label class="form-label">SÃ©ries</label><input class="form-input" name="tabata_sets" type="number" value="4" /></div>
              <div class="form-group"><label class="form-label">Descanso entre sÃ©ries (s)</label><input class="form-input" name="tabata_rest" type="number" value="60" /></div>
            </div>
          </div>`;
          ...
          html = `<div class="card mt-md" style="background:rgba(245,158,11,0.05);border-color:rgba(245,158,11,0.2)">
            ...
            <div class="form-row">
              ...
              <div class="form-group"><label class="form-label">RecuperaÃ§Ã£o (s)</label><input class="form-input" name="sit_rest" type="number" value="180" /></div>
